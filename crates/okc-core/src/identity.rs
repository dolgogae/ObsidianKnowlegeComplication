...
impl ContentHash {
    pub fn from_bytes(bytes: &[u8]) -> Self {
        Self::from_domain_bytes("okc:content:v2\0", bytes)
    }
    ...
    pub fn parse_hex(value: &str) -> Result<Self> {
        ...
        let decoded = hex::decode(value).map_err(|error| OkcError::MalformedInput {
            path: "identity".into(),
            reason: error.to_string(),
        })?;
        let bytes: [u8; 32] = decoded.try_into().map_err(|_| OkcError::MalformedInput {
            path: "identity".into(),
...
impl SnapshotId {
    ...
    pub fn from_manifest<'a>(
        ...
        let mut hasher = Sha256::new();
        hasher.update(b"okc:snapshot:v2\0");
