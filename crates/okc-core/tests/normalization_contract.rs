...
use okc_core::identity::{ContentHash, RecordId};
use okc_core::ir::{
    BlockKind, CanvasReferenceResolution, FileKind, LinkResolution, SourcePathEncoding,
};
use okc_core::plan::{ConflictKind, DraftPlan, OutputOperation, RewriteReplacement};
use okc_core::provenance::{ProvenanceRecord, ProvenanceRecordKind, SourceRecord};
use okc_core::{OkcCompiler, OkcError, SourceSpec};
...
fn assert_archive_path_rejected(compiler: &OkcCompiler, path: &Path, source_id: &str) {
    ...
    assert!(
        matches!(
            error,
            OkcError::UnsafePath { .. } | OkcError::MalformedInput { .. }
        ),
...
fn zip_outer_size_is_bounded_before_archive_parse() {
    ...
    let mut policy = okc_core::CompilerPolicy::default();
    ...
    assert!(
        matches!(error, OkcError::ResourceLimit(_)),
...
fn duplicate_or_invalid_utf8_pax_paths_fail_closed() {
    ...
    for (name, pax_paths) in [
        ...
        assert!(
            matches!(error, OkcError::MalformedInput { .. }),
...
fn read_stored_records(artifact: &Path) -> Vec<ProvenanceRecord> {
    let bytes =
        fs::read(artifact.join(".okc/provenance.jsonl")).expect("read stored provenance graph");
...
fn reseal_records(root: &Path, records: &mut [ProvenanceRecord]) {
    ...
    fs::write(root.join(".okc/provenance.jsonl"), &bytes)
...
fn spelling_only_source_rename_after_plan_is_stale_and_publishes_nothing() {
    ...
    assert!(
        matches!(
            error,
            OkcError::IdentityMismatch(_) | OkcError::PlanStale(_)
        ),
...
fn verifier_rejects_fully_resealed_original_path_provenance_tamper() {
    ...
    okc_core::provenance::validate_stored_graph(&records)
...
fn full_unicode_case_fold_collides_sharp_s_and_sigma_without_rewriting_spelling() {
    ...
    assert!(path_conflicts.iter().all(|conflict| {
        conflict.documents.len() == 2
            ...
            && conflict
                .resolution
                .eq(&okc_core::plan::ConflictResolution::AutoResolvedByNormativeRule)
    }));
    assert_eq!(
...
fn lone_cr_drives_markdown_line_fence_block_and_link_semantics_with_raw_spans() {
    ...
    let raw_span = |span: &okc_core::diagnostic::SourceSpan| {
...
fn canvas_normalization_vectors_resolve_nfd_preserve_unknowns_and_copy_unchanged_bytes() {
    ...
    assert_eq!(
        rewritten_output,
        okc_core::canonical::to_canonical_json_pretty(&output_value)
