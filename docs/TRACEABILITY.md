...
| REQ-PAR-002 JSON Canvas parsing and rewrite | [`canonical-knowledge-ir.md`](specs/canonical-knowledge-ir.md), ALG-NRM-001, ADR-0008 | `vaultc::parse::parse_canvas`, `ir::{Canvas, CanvasFileReference}`; current materialization copies Canvas | `compiled_layout_excludes_raw_sources_and_preserves_v1_artifacts`; `malformed_utf8_markdown_and_canvas_fail_closed` | implemented partially: parse/preserve only; reference resolution/rewrite and golden corpus pending |
...
| REQ-SEC-001 hostile input isolation | [`security-and-trust-boundaries.md`](specs/security-and-trust-boundaries.md), ADR-0003 | `vaultc::{snapshot,provider,approval,compile,pack,verify}` and CLI process/control-file guards | 15 cases in [`security_contract.rs`](../crates/vaultc/tests/security_contract.rs), provider approval tests, CLI bounded-I/O/cancellation tests | implemented; fuzz/property campaigns and platform matrix pending |
