...
joined with `/` before NFC. The current path contract requires `N_path(O_path(p))` to equal the sealed
...
The retained implementation pins NFC data to Unicode 17.0.0 and default full case
