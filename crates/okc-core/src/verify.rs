...
pub fn verify_artifact(path: &Path) -> Result<VerificationReport> {
        ...
        .is_some_and(|extension| extension.eq_ignore_ascii_case("okcpack"))
    {
        ...
        {
            let temporary = tempfile::tempdir().map_err(|error| OkcError::io(path, error))?;
        ...
        Err(OkcError::VerificationFailed(
            "artifact must be a Compiled Vault directory or .okcpack".into(),
        ))
    }
}
...
pub(crate) fn verify_directory(root: &Path) -> Result<VerificationReport> {
    ...
    let manifest_bytes =
        fs::read(&manifest_path).map_err(|error| OkcError::io(&manifest_path, error))?;
    let manifest: ArtifactManifest = serde_json::from_slice(&manifest_bytes)
        .map_err(|error| OkcError::VerificationFailed(format!("manifest is malformed: {error}")))?;
    ...
    if manifest.format_family != "okc" {
        return Err(OkcError::VerificationFailed(format!(
            "unsupported format family `{}`",
            manifest.format_family
        )));
    }
    ...
    let checksum_text =
        fs::read_to_string(&checksum_path).map_err(|error| OkcError::io(&checksum_path, error))?;
    ...
    for (line_number, line) in checksum_text.lines().enumerate() {
        ...
        validate_output_logical_path(path, &CompilerPolicy::default()).map_err(|error| {
            OkcError::VerificationFailed(format!("unsafe checksum path: {error}"))
        })?;
    ...
    for (logical, hash) in &expected {
        ...
        let metadata = fs::symlink_metadata(&file).map_err(|error| OkcError::io(&file, error))?;
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(OkcError::VerificationFailed(format!(
                "artifact member `{logical}` is not a regular file"
            )));
        }
        if logical == ".okc/provenance.jsonl"
        ...
        let bytes = fs::read(&file).map_err(|error| OkcError::io(&file, error))?;
    ...
    let inventory = crate::compile::inventory(root, &[".okc/checksums.txt"])?;
    ...
    let identity_inventory =
        crate::compile::inventory(root, &[".okc/manifest.json", ".okc/checksums.txt"])?;
    ...
    let approved_path = root.join(".okc/plan.json");
    let approved_bytes =
        fs::read(&approved_path).map_err(|error| OkcError::io(&approved_path, error))?;
    let approved: crate::approval::ApprovedPlan =
        serde_json::from_slice(&approved_bytes).map_err(|error| {
            OkcError::VerificationFailed(format!("sealed approved plan is malformed: {error}"))
        })?;
    ...
    crate::approval::validate_approved_plan(&approved, &approved.plan.policy)
        .map_err(|error| OkcError::VerificationFailed(error.to_string()))?;
    ...
    let proposal_hashes: Vec<_> = approved
        ...
        .collect();
    let proposal_ids: Vec<_> = approved
        .approved_proposals
        .iter()
        .map(|proposal| proposal.proposal.proposal_id.clone())
        .collect();
    if manifest.plan_id != approved.plan.plan_id.to_string()
        || manifest.materialization_id != approved.materialization.materialization_id.to_string()
        ...
        || manifest.approved_proposal_ids != proposal_ids
        || manifest.distribution.pack_profile != crate::pack::PACK_PROFILE
        || manifest.distribution.user_pack_publisher_signed
    {
        return Err(OkcError::VerificationFailed(
    ...
    let provenance = checked_provenance.ok_or_else(|| {
        OkcError::VerificationFailed("stored provenance graph was not checksummed".into())
    })?;
...
fn validate_audit_files(root: &Path, approved: &crate::approval::ApprovedPlan) -> Result<()> {
    ...
    let transcript_path = root.join(".okc/ai-transcript.jsonl");
    let transcript =
        fs::read(&transcript_path).map_err(|error| OkcError::io(&transcript_path, error))?;
    ...
    let conflicts_path = root.join(".okc/conflicts.json");
    ...
    if fs::read(&conflicts_path).map_err(|error| OkcError::io(&conflicts_path, error))?
        != expected_conflicts
    {
        return Err(OkcError::VerificationFailed(
    ...
    let diagnostics_path = root.join(".okc/diagnostics.json");
    let expected_diagnostics =
        crate::canonical::to_canonical_json_pretty(&approved.plan.diagnostics)?;
    if fs::read(&diagnostics_path).map_err(|error| OkcError::io(&diagnostics_path, error))?
...
fn validate_operation_output_hashes(
    ...
    for operation in &approved.materialization.effective_operations {
        ...
        let actual = fs::read(&path).map_err(|error| OkcError::io(&path, error))?;
...
fn validate_generated_outputs(
    ...
    for proposal in &approved.approved_proposals {
        ...
        match (&proposal.proposal.kind, &proposal.materialization) {
            (
                okc_protocol::ProposalKind::CreateGeneratedNote { markdown_body, .. },
                ...
                let actual =
                    fs::read(&output_path).map_err(|error| OkcError::io(&output_path, error))?;
            ...
            (
                okc_protocol::ProposalKind::ExplainConflict { .. },
...
fn reconstruct_markdown_source(
    ...
    let source_len = usize::try_from(document.source_file.byte_len).map_err(|_| {
        OkcError::VerificationFailed("sealed Markdown source length overflows usize".into())
    })?;
    ...
    for replacement in replacements {
        let start = usize::try_from(replacement.span.byte_start).map_err(|_| {
            OkcError::VerificationFailed("Markdown replacement start overflows usize".into())
        })?;
        let end = usize::try_from(replacement.span.byte_end).map_err(|_| {
            OkcError::VerificationFailed("Markdown replacement end overflows usize".into())
        })?;
        ...
        let unchanged_end = output_cursor.checked_add(unchanged_len).ok_or_else(|| {
            OkcError::VerificationFailed("Markdown output offset overflow".into())
        })?;
        ...
        let unchanged = output
            .get(output_cursor..unchanged_end)
            .ok_or_else(|| OkcError::VerificationFailed("Markdown output is truncated".into()))?;
    ...
    let remaining = source_len
        ...
        .ok_or_else(|| OkcError::VerificationFailed("Markdown source offset underflow".into()))?;
...
fn validate_rewritten_markdown_links(
    ...
    output_file.byte_len = u64::try_from(output.len()).map_err(|_| {
        OkcError::VerificationFailed("rewritten Markdown length overflows u64".into())
    })?;
...
fn validate_artifact_tree(root: &Path) -> Result<()> {
        ...
        .build()
    {
        ...
        let relative = entry
            .path()
            .strip_prefix(root)
            .map_err(|_| OkcError::VerificationFailed("artifact entry escaped its root".into()))?;
        let logical = relative
            .components()
            .map(|component| {
                component.as_os_str().to_str().ok_or_else(|| {
                    OkcError::VerificationFailed("artifact contains a non-UTF-8 path".into())
                })
            })
            .collect::<Result<Vec<_>>>()?
            .join("/");
        validate_output_logical_path(&logical, &CompilerPolicy::default()).map_err(|error| {
            OkcError::VerificationFailed(format!("unsafe artifact path: {error}"))
        })?;
