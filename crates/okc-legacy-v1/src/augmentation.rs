...
pub const AUGMENTATION_SCHEMA_VERSION: u32 = 1;
...
pub struct RecordedAugmentation {
    ...
    plan_id: PlanId,
    projection_hash: ContentHash,
    provider: vaultc_protocol::ProviderIdentity,
...
enum AugmentationRecordRef<'a> {
    Header {
        ...
        plan_id: PlanId,
        projection_hash: ContentHash,
        provider: &'a vaultc_protocol::ProviderIdentity,
    },
    Transcript {
...
impl RecordedAugmentation {
    ...
    #[must_use]
    pub fn provider(&self) -> &vaultc_protocol::ProviderIdentity {
        &self.provider
    }
    ...
    for projection in &request.documents {
        let document_id = projection.document.id.parse::<DocumentId>().map_err(|_| {
            VaultcError::ApprovalStale(
                "augmentation request contains a malformed document ID".into(),
            )
        })?;
...
fn authorize_replayed_provider(
    ...
    validate_capabilities(capabilities, policy).map_err(|error| {
        VaultcError::ApprovalStale(format!("recorded provider is not permitted: {error}"))
    })?;
    validate_capability_identity(capabilities).map_err(|error| {
        VaultcError::ApprovalStale(format!("recorded provider identity is invalid: {error}"))
    })?;
    if !capabilities.structured_output {
        return Err(VaultcError::ApprovalStale(
...
fn ensure_payload_within_limit<T: Serialize>(
    ...
    let length = u64::try_from(to_canonical_json(payload)?.len()).map_err(|_| {
        VaultcError::ResourceLimit("augmentation payload byte length overflow".into())
    })?;
    if length > limit {
...
fn provider_error(message: impl Into<String>) -> VaultcError {
    VaultcError::Provider(message.into())
}
