...
impl CompilerPolicy {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.paths.follow_symlinks {
            return Err(VaultcError::InvalidConfig(
                "following source symlinks is not supported by the V1 safety policy".into(),
            ));
        }
        if !self.output.reject_existing_destination {
            return Err(VaultcError::InvalidConfig(
                "V1 always rejects an existing destination; an update workflow requires a new ADR"
                    .into(),
            ));
        }
        ...
        {
            return Err(VaultcError::InvalidConfig(
                "V1 identity policy requires 128 MinHash components in 32x4 bands".into(),
            ));
        }
    ...
    pub fn semantic_hash(&self) -> Result<crate::identity::ContentHash> {
        crate::canonical::canonical_hash("vaultc:policy:v1\0", self)
    }
}
...
mod tests {
    ...
    #[test]
    fn v1_cannot_disable_existing_destination_rejection() {
