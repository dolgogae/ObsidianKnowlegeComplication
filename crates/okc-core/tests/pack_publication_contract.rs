...
use okc_core::{ApprovedPlan, CompileOptions, OkcCompiler, OkcError, SourceSpec};

fn approved_basic_plan() -> (OkcCompiler, ApprovedPlan) {
...
fn compile_basic(compiler: &OkcCompiler, approved: &ApprovedPlan, destination: &Path) {
    compiler
...
fn assert_output_exists(error: &OkcError, destination: &Path) {
    assert!(
        matches!(error, OkcError::OutputExists(path) if path == destination),
...
fn sdk_pack_existing_destination_is_preserved() {
    ...
    let existing_file = temporary.path().join("existing-file.okcpack");
    ...
    let file_error = okc_core::pack::create_pack(
        &compiled,
        &existing_file,
    ...
    let existing_directory = temporary.path().join("existing-directory.okcpack");
    ...
    let directory_error = okc_core::pack::create_pack(
        &compiled,
        &existing_directory,
        compiler.policy().output.zstd_level,
    )
...
fn sdk_pack_dangling_symlink_is_rejected_without_following_target() {
    ...
    let dangling_link = temporary.path().join("dangling.okcpack");
    ...
    let dangling_error = okc_core::pack::create_pack(
        &compiled,
        &dangling_link,
        compiler.policy().output.zstd_level,
    )
...
fn sdk_pack_destination_inside_compiled_vault_is_rejected_without_mutation() {
    ...
    let direct_inside = compiled.join("inside.okcpack");
    let direct_error = okc_core::pack::create_pack(
        &compiled,
        &direct_inside,
        compiler.policy().output.zstd_level,
    )
    ...
    assert!(
        matches!(direct_error, OkcError::UnsafePath { .. }),
    ...
    let lexical_inside = compiled.join("nested/../lexical.okcpack");
    let lexical_error = okc_core::pack::create_pack(
        &compiled,
        &lexical_inside,
        compiler.policy().output.zstd_level,
    )
    ...
    assert!(
        matches!(lexical_error, OkcError::UnsafePath { .. }),
        "lexical containment must be classified as unsafe: {lexical_error:?}"
    );
    assert!(!compiled.join("lexical.okcpack").exists());
    ...
    #[cfg(unix)]
    {
        ...
        let aliased_inside = compiled_alias.join("aliased.okcpack");
        let alias_error = okc_core::pack::create_pack(
            &compiled,
            &aliased_inside,
            compiler.policy().output.zstd_level,
        )
        ...
        assert!(
            matches!(alias_error, OkcError::UnsafePath { .. }),
            "resolved-ancestor containment must be classified as unsafe: {alias_error:?}"
        );
        assert!(!compiled.join("aliased.okcpack").exists());
    }
    assert_eq!(common::tree_bytes(&compiled), compiled_before);
}
...
fn sdk_pack_equal_and_vault_inside_reserved_pack_path_are_rejected() {
    ...
    let equal = temporary.path().join("equal.okcpack");
    compile_basic(&compiler, &approved, &equal);
    let equal_before = common::tree_bytes(&equal);
    let equal_error =
        okc_core::pack::create_pack(&equal, &equal, compiler.policy().output.zstd_level)
            .expect_err("equal compiled-Vault and pack paths must be rejected");
    assert!(matches!(
        equal_error,
        OkcError::UnsafePath { .. } | OkcError::OutputExists(_)
    ));
    assert_eq!(common::tree_bytes(&equal), equal_before);

    let reserved_pack = temporary.path().join("reserved.okcpack");
    ...
    let reverse_error = okc_core::pack::create_pack(
        &nested_compiled,
        &reserved_pack,
        compiler.policy().output.zstd_level,
    )
...
fn sdk_pack_portable_case_and_nfc_aliases_do_not_bypass_containment() {
    ...
    for (compiled_spelling, alias_spelling) in cases {
        ...
        let aliased_inside = temporary
            .path()
            .join(alias_spelling)
            .join("portable-alias.okcpack");
        let error = okc_core::pack::create_pack(
            &compiled,
            &aliased_inside,
            compiler.policy().output.zstd_level,
        )
        ...
        assert!(
            matches!(error, OkcError::UnsafePath { .. }),
...
fn sdk_pack_failure_does_not_leave_partial_destination() {
    ...
    let compiled_before = common::tree_bytes(&compiled);
    let pack = temporary.path().join("rejected.okcpack");
    let entries_before = entry_names(temporary.path());

    let error = okc_core::pack::create_pack(&compiled, &pack, compiler.policy().output.zstd_level)
    ...
    okc_core::pack::create_pack(&compiled, &pack, compiler.policy().output.zstd_level)
...
fn sdk_pack_concurrent_publish_has_exactly_one_winner() {
    ...
    compile_basic(&compiler, &approved, &compiled);
    let compiled_before = common::tree_bytes(&compiled);
    let pack = temporary.path().join("winner.okcpack");
    let entries_before = entry_names(temporary.path());
    ...
    let handles = (0..CREATOR_COUNT)
        .map(|_| {
            ...
            thread::spawn(move || {
                barrier.wait();
                okc_core::pack::create_pack(&compiled, &pack, zstd_level)
            })
        })
        .collect::<Vec<_>>();
    let results = handles
...
fn compile_options_reject_output_pack_aliases_before_publication() {
    ...
    let cases = [
        ...
        (
            ...
            temporary
                .path()
                .join("output-with-inner-pack/inner.okcpack"),
        ),
        (
            temporary.path().join("outer.okcpack/compiled"),
            temporary.path().join("outer.okcpack"),
        ),
        (
            temporary.path().join("CaféOutput"),
            temporary.path().join("CAFE\u{301}OUTPUT/inner.okcpack"),
        ),
        (
            temporary.path().join("StraßeOutput"),
            temporary.path().join("STRASSEOUTPUT/inner.okcpack"),
        ),
    ];
...
fn compile_with_existing_pack_fails_preflight_and_leaves_output_absent() {
    ...
    let pack = temporary.path().join("existing.okcpack");
...
fn separate_and_compile_with_options_publish_byte_identical_results() {
    ...
    let separate_pack = temporary.path().join("separate.okcpack");
    let integrated_pack = temporary.path().join("integrated.OKCPACK");
...
fn compile_with_pack_failure_leaves_valid_compiled_vault_and_no_partial_pack() {
    ...
    let pack = pack_parent.join("raced.okcpack");
    ...
    match error {
        OkcError::PackPublicationAfterCompile {
            compiled_vault,
            pack: failed_pack,
            ..
        } => {
    ...
    assert!(
        fs::symlink_metadata(&pack).is_err(),
        "okc must leave no requested pack or partial pack"
    );
    assert_eq!(
