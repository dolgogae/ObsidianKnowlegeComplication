...
pub(crate) fn apply_markdown_target_selection(
    plan: &DraftPlan,
    operations: &mut [OutputOperation],
    document_id: DocumentId,
    link_id: LinkId,
    target_document_id: DocumentId,
    committed_operations: Option<&[OutputOperation]>,
) -> Result<()> {
    let document = plan.workspace.documents.get(&document_id).ok_or_else(|| {
        OkcError::ApprovalStale(format!("selected Markdown document {document_id} is stale"))
    })?;
    let link = document
        .links
        .iter()
        .find(|link| link.link_id == link_id)
        .ok_or_else(|| OkcError::ApprovalStale(format!("selected link {link_id} is stale")))?;
    let LinkResolution::Ambiguous { candidates } = &link.resolution else {
        return Err(OkcError::ApprovalStale(format!(
            "selected link {link_id} is no longer ambiguous"
        )));
    };
    if !candidates.contains(&target_document_id) {
        return Err(OkcError::ApprovalStale(format!(
            "target {target_document_id} is outside link {link_id}'s sealed candidates"
        )));
    }
    let destination = plan
        .output_paths
        .get(&document_id)
        .ok_or_else(|| OkcError::PlanStale(format!("document {document_id} has no output path")))?;
    let target_output = plan.output_paths.get(&target_document_id).ok_or_else(|| {
        OkcError::PlanStale(format!("target {target_document_id} has no output path"))
    })?;
    let replacement = RewriteReplacement {
        span: link.span.clone(),
        replacement: render_link_target(link, &relative_output_target(destination, target_output)),
    };
    let operation = operations
        ...
        .ok_or_else(|| {
            OkcError::PlanStale(format!(
                "document {document_id} has no materialized operation"
            ))
        })?;
    let mut replacements = match operation {
        OutputOperation::Copy { .. } => Vec::new(),
        OutputOperation::RewriteMarkdown { replacements, .. } => replacements.clone(),
        OutputOperation::RewriteCanvas { .. } => {
            return Err(OkcError::PlanStale(format!(
                "document {document_id} maps to a non-Markdown operation"
            )));
        }
    };
    if replacements
        .iter()
        .any(|existing| existing.span == replacement.span)
    {
        return Err(OkcError::ApprovalStale(format!(
            "link {link_id} received more than one target action"
        )));
    }
    replacements.push(replacement);
    replacements.sort_by_key(|item| item.span.byte_start);
    validate_markdown_replacement_structure(document.source_file.byte_len, &replacements)?;

    let expected_output_hash = selected_markdown_output_hash(
        plan,
        document,
        destination,
        &replacements,
    ...
    *operation = OutputOperation::RewriteMarkdown {
        operation_id,
...
fn selected_markdown_output_hash(
    plan: &DraftPlan,
    ...
    if let Some(committed_operations) = committed_operations {
        let committed = committed_operations
            .iter()
            .find(|operation| operation.destination() == destination)
            .ok_or_else(|| {
                OkcError::ApprovalStale(format!(
                    "committed materialization has no operation for `{destination}`"
                ))
            })?;
        match committed {
            OutputOperation::RewriteMarkdown {
                ...
                expected_output_hash,
                ..
            } if source_id == &document.source_file.source_id
                && *snapshot_id == document.source_file.snapshot_id
                && source_path == &document.source_file.logical_path
                && *expected_hash == document.source_file.content_hash =>
            {
                Ok(*expected_output_hash)
            }
            _ => Err(OkcError::ApprovalStale(format!(
                "committed materialization operation for `{destination}` is stale"
            ))),
        }
    } else {
        let snapshot = plan
            .snapshots
            .iter()
            .find(|snapshot| snapshot.snapshot_id == document.source_file.snapshot_id)
            .ok_or_else(|| OkcError::PlanStale("selected Markdown snapshot is stale".into()))?;
        let source = crate::snapshot::read_source_entry(
            &snapshot.source,
            &document.source_file,
            &plan.policy,
        )?;
        let rendered =
            render_rewritten_markdown(document, destination, &source, replacements, &plan.policy)?;
        Ok(ContentHash::from_bytes(&rendered))
    }
}

pub(crate) fn apply_canvas_target_selection(
    plan: &DraftPlan,
    operations: &mut [OutputOperation],
    canvas_id: CanvasId,
    node_id: &str,
    ...
    let canvas =
        plan.workspace.canvases.get(&canvas_id).ok_or_else(|| {
            OkcError::ApprovalStale(format!("selected Canvas {canvas_id} is stale"))
        })?;
    let reference = canvas
        .file_references
        .iter()
        ...
        .ok_or_else(|| {
            OkcError::ApprovalStale(format!("selected Canvas node `{node_id}` is stale"))
        })?;
    let CanvasReferenceResolution::Ambiguous { candidates } = &reference.resolution else {
        return Err(OkcError::ApprovalStale(format!(
            "selected Canvas node `{node_id}` is no longer ambiguous"
        )));
    };
    if !candidates.contains(&target) {
        return Err(OkcError::ApprovalStale(format!(
            "Canvas target is outside node `{node_id}`'s sealed candidates"
        )));
    }
    let destination = plan
        .canvas_output_paths
        .get(&canvas_id)
        .ok_or_else(|| OkcError::PlanStale(format!("Canvas {canvas_id} has no output path")))?;
    ...
    .ok_or_else(|| OkcError::PlanStale("selected Canvas target has no output path".into()))?;
    let rewrite = CanvasReferenceRewrite {
        node_id: node_id.to_owned(),
        original_path: reference.raw_path.clone(),
        replacement_path: relative_output_target(destination, target_output),
        target,
    };
    let operation = operations
        .iter_mut()
        .find(|operation| operation.destination() == destination)
        .ok_or_else(|| {
            OkcError::PlanStale(format!("Canvas {canvas_id} has no materialized operation"))
        })?;
    let mut rewrites = match operation {
        OutputOperation::Copy { .. } => Vec::new(),
        OutputOperation::RewriteCanvas { rewrites, .. } => rewrites.clone(),
        OutputOperation::RewriteMarkdown { .. } => {
            return Err(OkcError::PlanStale(format!(
                "Canvas {canvas_id} maps to a non-Canvas operation"
            )));
        }
    };
    if rewrites.iter().any(|existing| existing.node_id == node_id) {
        return Err(OkcError::ApprovalStale(format!(
            "Canvas node `{node_id}` received more than one target action"
        )));
    }
    rewrites.push(rewrite);
    rewrites.sort_by(|left, right| left.node_id.as_bytes().cmp(right.node_id.as_bytes()));
    let rendered = render_rewritten_canvas(canvas, &rewrites)?;
    let expected_output_hash = ContentHash::from_bytes(&rendered);
    let operation_id = canvas_rewrite_operation_id(
        &canvas.source_file,
        destination,
        expected_output_hash,
    ...
    *operation = OutputOperation::RewriteCanvas {
        operation_id,
        source_id: canvas.source_file.source_id.clone(),
        snapshot_id: canvas.source_file.snapshot_id,
        source_path: canvas.source_file.logical_path.clone(),
