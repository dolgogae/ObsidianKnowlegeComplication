...
use okc_core::ir::{Canvas, CanvasFileReference, CanvasReferenceResolution, CanvasReferenceTarget};
use okc_core::plan::{ConflictKind, ConflictSubject, DraftPlan};
use okc_core::{ApprovalLog, OkcError, SourceSpec, ValidatedProposals};
...
fn ambiguous_canvas_reference_requires_waiver_and_preserves_raw_path() {
    ...
    let decisions: Vec<_> = conflicts
        ...
        .map(|conflict| DecisionOverlay {
            ...
            decided_by: CuratorId::new("canvas-qa").expect("valid curator ID"),
    ...
    let selection_decisions: Vec<_> = conflicts
        .iter()
        .map(|conflict| {
            let Some(ConflictSubject::CanvasReference { node_id, .. }) = &conflict.subject else {
                panic!("expected Canvas conflict subject")
            };
            let CanvasReferenceResolution::Ambiguous { candidates } =
                &canvas_reference(source_canvas, node_id).resolution
            else {
                panic!("expected sealed Canvas candidates")
            };
            DecisionOverlay {
                plan_id: plan.plan_id.to_string(),
                conflict_id: conflict.conflict_id.clone(),
                conflict_content_hash: conflict.content_hash,
                action: ConflictAction::SelectCanvasTarget {
                    target: candidates[0],
                },
                decided_by: CuratorId::new("canvas-qa").expect("valid curator ID"),
                policy_version: "test-v2".into(),
                rationale: Some("select one sealed Canvas target".into()),
            }
        })
        .collect();
    let selected = compiler
        ...
        .expect("select both Canvas ambiguity targets");
    assert_eq!(
        selected.plan.operations, plan.operations,
        "DraftPlan is immutable"
    );
    assert_ne!(
        selected.materialization.effective_operations, plan.operations,
        "typed selections derive effective operations"
    );
    let selected_output = temporary.path().join("compiled-selected");
    compiler
        .compile(&selected, &selected_output)
        .expect("compile selected Canvas targets");
    compiler
        .verify(&selected_output)
        .expect("verify selected Canvas targets");
    let selected_value: Value = serde_json::from_slice(
        &fs::read(selected_output.join(&canvas_output)).expect("read selected Canvas"),
    )
    .expect("parse selected Canvas");
    for node_id in ["ambiguous-node-a", "ambiguous-node-b"] {
        let CanvasReferenceResolution::Ambiguous { candidates } =
            &canvas_reference(source_canvas, node_id).resolution
        else {
        ...
        let CanvasReferenceTarget::Document(document_id) = candidates[0] else {
            panic!("fixture ambiguity must resolve to documents")
        };
        let target_output = plan
            .output_paths
            .get(&document_id)
            .expect("selected target output");
        assert_eq!(
            canvas_node_file(&selected_value, node_id),
            relative_output_target(&canvas_output, target_output)
        );
        assert_eq!(
            canvas_node(&selected_value, node_id)["fixture_unknown"],
            canvas_node(&canvas_value, node_id)["fixture_unknown"],
            "typed selection must preserve unknown Canvas fields"
        );
    }
}
...
fn unsafe_canvas_reference_that_escapes_output_root_fails_closed() {
    ...
    let inspection =
        match compiler.inspect([common::fixture_source("canvas-unsafe", "canvas_unsafe")]) {
            Err(OkcError::UnsafePath { .. } | OkcError::MalformedInput { .. }) => {
...
fn duplicate_canvas_node_ids_fail_closed() {
    ...
    let inspection = match compiler.inspect([common::fixture_source(
        ...
        Err(OkcError::MalformedInput { .. }) => return,
