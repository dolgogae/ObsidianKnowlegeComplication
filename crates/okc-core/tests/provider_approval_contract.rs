...
use okc_core::identity::ContentHash;
use okc_core::provenance::ProvenanceRecordKind;
use okc_core::{ApprovalDecision, ApprovalLog, DraftPlan, OkcError};
use okc_protocol::{
...
fn basic_plan() -> (okc_core::OkcCompiler, DraftPlan) {
...
fn validate_evidence_case(
    compiler: &okc_core::OkcCompiler,
    plan: &DraftPlan,
    proposal_id: &str,
    evidence: EvidenceRefWire,
) -> okc_core::ProposalValidation {
...
fn valid_proposal_requires_explicit_matching_approval() {
    ...
    assert_eq!(frontmatter["okc_generated"], true);
    assert_eq!(frontmatter["okc_proposal_id"], "proposal-fixture-1");
...
fn approved_proposal_content_cannot_change_before_materialization() {
    ...
    assert!(matches!(
        error,
        OkcError::ApprovalStale(_) | OkcError::ProposalInvalid(_)
    ));
