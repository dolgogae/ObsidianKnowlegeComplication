...
pub fn validate_capabilities(
    ...
    if !capabilities.supports(ProviderOperation::KnowledgeAugmentation) {
        return Err(OkcError::Provider(
            "provider does not support protocol V2 knowledge augmentation".into(),
        ));
    }
...
pub fn validate_proposals(
    ...
    for proposal in proposals {
        let content_hash = canonical_hash("okc:proposal:v2\0", &proposal)?;
...
fn validate_provider_identity(
    identity: &okc_protocol::ProviderIdentity,
