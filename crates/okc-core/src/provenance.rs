...
use okc_protocol::{ProposalKindName, ProviderIdentity};
...
use crate::approval::{ApprovalDecision, ApprovedPlan, DecisionOverlay, ProposalMaterialization};
...
pub fn stored_graph_hash(jsonl: &[u8]) -> ContentHash {
    ContentHash::from_domain_bytes("okc:provenance-graph:v2\0", jsonl)
}

fn explanation_graph_hash(jsonl: &[u8]) -> ContentHash {
    ContentHash::from_domain_bytes("okc:provenance-explanation:v2\0", jsonl)
}
    ...
    match subject {
        ProvenanceSubject::ArtifactPath { path } => {
            ...
            let len = u64::try_from(path.len()).map_err(|_| {
                OkcError::ResourceLimit("provenance subject path length overflow".into())
            })?;
...
fn cursor_value(
    ...
    hasher.update(b"okc:provenance-cursor:v2\0");
...
pub(crate) fn encode_jsonl(records: &[ProvenanceRecord]) -> Result<Vec<u8>> {
    ...
    for record in records {
        ...
        let next_len = output
            ...
            .ok_or_else(|| OkcError::ResourceLimit("provenance graph length overflow".into()))?;
...
pub(crate) fn decode_jsonl(bytes: &[u8], path: &Path) -> Result<Vec<ProvenanceRecord>> {
        ...
        .enumerate()
    {
        ...
        let record: ProvenanceRecord =
            serde_json::from_value(value).map_err(|error| OkcError::MalformedInput {
                path: path.display().to_string(),
                reason: format!("line {}: {error}", index + 1),
            })?;
...
pub(crate) fn generated_output_path(proposal: &okc_protocol::KnowledgeProposal) -> Option<String> {
    match &proposal.kind {
        okc_protocol::ProposalKind::CreateGeneratedNote {
            title,
            suggested_path,
            ..
        } => {
        ...
        okc_protocol::ProposalKind::ExplainConflict { .. } => None,
    }
}
...
fn sanitize_generated_name(title: &str, proposal_id: &str) -> String {
    ...
    let suffix = ContentHash::from_domain_bytes("okc:proposal-path:v2\0", proposal_id.as_bytes());
...
fn attribution_state(document: &crate::ir::Document) -> Result<AttributionState> {
    let absent_hash = ContentHash::from_domain_bytes("okc:frontmatter:v2\0", b"null");
...
fn evidence_source(
    ...
    evidence: &okc_protocol::EvidenceRefWire,
) -> Result<SourceRecord> {
            ...
            builder.edge(
                ...
                EdgePosition::Ordered {
                    index: u64::try_from(index).map_err(|_| {
                        OkcError::ResourceLimit("proposal evidence count overflow".into())
                    })?,
                },
            )?;
...
fn add_planned_output(
    ...
    let source_count = u64::try_from(sources.source_records.len())
        .map_err(|_| OkcError::ResourceLimit("provenance source count overflow".into()))?;
...
fn add_stored_audit_outputs(
    ...
    for path in STORED_AUDIT_PATHS {
        let file = files.get(path).ok_or_else(|| {
            OkcError::IdentityMismatch(format!("stored audit output `{path}` is missing"))
        })?;
...
pub(crate) fn build_stored_records(
    ...
    if actual_paths != expected_paths {
        return Err(OkcError::IdentityMismatch(
            "stored graph inventory does not exactly cover pre-envelope outputs".into(),
        ));
    }

    let transcript_hash = files[".okc/ai-transcript.jsonl"].content_hash;
    ...
    let plan_input = add_build_input(
        ...
        files[".okc/plan.json"].content_hash,
    )?;
...
fn validate_source_record(source: &SourceRecord) -> Result<()> {
    match source {
        ...
        SourceRecord::Evidence(source) => {
            ...
            if EvidenceId::from_evidence(&okc_protocol::EvidenceRefWire {
...
fn validate_output_record(output: &OutputRecord, mode: GraphStorageMode) -> Result<()> {
    match (&output.subject, output.storage, output.role, mode) {
        ...
        _ => Err(OkcError::VerificationFailed(
    ...
    let valid = match operation {
        ...
        OperationRecord::Generate {
            ...
            proposals.len() == 1
                ...
                && proposals[0].proposal_kind == okc_protocol::ProposalKindName::CreateGeneratedNote
                && proposals[0].valid
...
fn validate_graph_topology(
    ...
    for (_, edge) in edge_records {
        *indegree.get_mut(&edge.to).ok_or_else(|| {
            OkcError::VerificationFailed("edge target missing from DAG index".into())
        })? += 1;
    }
...
fn add_virtual_output(
    ...
    let output_id = builder.insert(ProvenanceRecordKind::Output(OutputRecord {
        ...
        byte_len: u64::try_from(bytes.len()).map_err(|_| {
            OkcError::ResourceLimit(format!("audit envelope `{path}` length overflow"))
        })?,
...
pub(crate) fn build_logical_graph(
    ...
    let provenance_bytes =
        fs::read(&provenance_path).map_err(|error| OkcError::io(&provenance_path, error))?;
    let manifest_bytes =
        fs::read(&manifest_path).map_err(|error| OkcError::io(&manifest_path, error))?;
    let checksums_bytes =
        fs::read(&checksums_path).map_err(|error| OkcError::io(&checksums_path, error))?;
...
fn induced_explanation(
    ...
    let root = root.ok_or_else(|| {
        OkcError::VerificationFailed(match subject {
            ProvenanceSubject::ArtifactPath { path } => {
...
fn package_graph(pack: &Path, extracted: &Path) -> Result<Vec<ProvenanceRecord>> {
    ...
    let manifest_path = extracted.join(".okc/manifest.json");
    let manifest_bytes =
        fs::read(&manifest_path).map_err(|error| OkcError::io(&manifest_path, error))?;
    let manifest: crate::compile::ArtifactManifest = serde_json::from_slice(&manifest_bytes)
        .map_err(|error| OkcError::VerificationFailed(format!("manifest is malformed: {error}")))?;
...
fn complete_explanation(
    ...
    match (artifact_is_pack(artifact), subject) {
        (false, ProvenanceSubject::Package) => Err(OkcError::VerificationFailed(
            "a package provenance subject requires a .okcpack artifact".into(),
        )),
        ...
        (true, _) => {
            ...
            {
                let temporary =
                    tempfile::tempdir().map_err(|error| OkcError::io(artifact, error))?;
            ...
            {
                Err(OkcError::UnsupportedSource(artifact.to_path_buf()))
            }
        }
    }
}
...
fn canonical_page_size(
    ...
    empty
        ...
        .ok_or_else(|| OkcError::ResourceLimit("provenance page length overflow".into()))
}
