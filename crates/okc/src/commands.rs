//! Current-schema CLI commands backed by shared application services.
...
use okc_app::project_state::{ClusterTaskOutput, TaskStage, TaskStatus, TaxonomyTaskOutput};
use okc_app::{ProjectStore, SourceBinding};
use okc_core::integration::{
    ApprovedClusterRevision, ApprovedIntegrationPlan, CriticSeverity, verify,
};
...
pub fn provider_command(command: ProviderCommand) -> CliResult<()> {
    ...
    match command {
        ...
        ProviderCommand::Test { name } => {
            ...
            println!("provider profile `{name}` passed current synthetic capability tests");
            Ok(())
        }
    }
}
...
fn latest_taxonomy_output(
    ...
    run: &okc_app::project_state::RunRecord,
...
pub fn compile_command(
    ...
    let artifact = if let Some(project_path) = selected {
        ...
        okc_core::integration::compile(&plan, output)
            .map_err(|error| CliFailure::from_error(EXIT_OUTPUT, error))?
    };
    verify(&artifact.path).map_err(|error| CliFailure::from_error(EXIT_VERIFY, error))?;
    match format {
        ...
        OutputFormat::Human => {
            println!(
                "compiled and independently verified Vault: {}",
