...
fn approved_directory_source(source_id: &str, source: &Path) -> (OkcCompiler, ApprovedPlan) {
...
fn compile_with_pack_has_exact_success_and_preflight_residual_states() {
    ...
    let successful_pack = temporary.path().join("successful.okcpack");
    compiler
        .compile_with_options(
            &approved,
            &successful_output,
    ...
    let skipped_pack = temporary.path().join("must-not-start.okcpack");
    ...
    let existing_pack = temporary.path().join("external-pack-winner.okcpack");
...
fn compile_with_pack_rejects_pack_inside_or_aliased_into_immutable_source() {
    ...
    let nested_pack = source.join("assets/must-not-create.okcpack");
    ...
    #[cfg(unix)]
    {
        ...
        let aliased_pack = source_alias.join("must-not-create.okcpack");
