...
use vaultc::identity::{ContentHash, EvidenceId, RecordId};
use vaultc::plan::{ConflictKind, ConflictResolution, ConflictSubject};
...
use vaultc::{
    ApprovalDecision, ApprovalLog, ApprovedPlan, DraftPlan, ValidatedProposals, VaultCompiler,
    VaultcError,
};
use vaultc_protocol::{
...
fn approved_generated_fixture_for(
    source_id: &str,
) -> (VaultCompiler, ApprovedPlan, KnowledgeProposal) {
...
fn read_stored_records(artifact: &std::path::Path) -> (Vec<u8>, Vec<ProvenanceRecord>) {
    let bytes = fs::read(artifact.join(".vaultc/provenance.jsonl"))
...
fn encode_records(records: &[ProvenanceRecord]) -> Vec<u8> {
    ...
    for record in records {
        bytes.extend(
            vaultc::canonical::to_canonical_json(record).expect("encode typed graph record"),
        );
        bytes.push(b'\n');
    }
    bytes
...
fn assert_graph_rejected(compiler: &VaultCompiler, artifact: &Path, attack: &str) {
...
fn typed_record_and_edge_ids_match_literal_vectors() {
    let source_kind =
        ProvenanceRecordKind::Source(SourceRecord::BuildInput(BuildInputSourceRecord {
            ...
            identity: "fixture-toolchain-v1".into(),
    ...
    assert_eq!(
        String::from_utf8(
            vaultc::canonical::to_canonical_json(&source_identity)
        ...
        concat!(
            ...
            "\"identity\":\"fixture-toolchain-v1\",\"input_kind\":\"toolchain\"}}},",
            "\"schema_version\":1}"
        )
    );
    ...
    assert_eq!(
        ...
        "record_f1f675cef79b22a5af2ff0979bb3be7535df30b518bb1784e20b680142ffa7e3"
    );
    ...
    assert_eq!(
        ...
        "record_7dfc96d71c7384a161e2dee36cf9ed52fbf4d542cc00c64d76de565438e6f0ff"
    );
...
fn stored_graph_closes_copy_rewrite_generated_admin_and_attribution() {
    ...
    vaultc::provenance::validate_stored_graph(&records).expect("validate stored typed graph");
    assert_strict_record_order(&records);
    assert_every_output_has_exact_producer(&records);

    let manifest: vaultc::compile::ArtifactManifest = serde_json::from_slice(
        &fs::read(artifact.join(".vaultc/manifest.json")).expect("read typed manifest"),
    )
    .expect("decode typed manifest");
    assert_eq!(manifest.provenance_schema_version, 1);
    assert_eq!(
        manifest.provenance_graph_hash,
        vaultc::provenance::stored_graph_hash(&jsonl)
    );
...
fn stored_graph_schema_order_ids_and_legacy_forms_fail_closed_after_reseal() {
    ...
    let original =
        fs::read(baseline.join(".vaultc/provenance.jsonl")).expect("read strict graph baseline");
    ...
    let mutate_first = |mutate: fn(&mut serde_json::Map<String, serde_json::Value>)| {
        ...
        let mut bytes = vaultc::canonical::to_canonical_json(&value)
...
fn verifier_rejects_fully_resealed_typed_graph_semantic_attacks() {
    ...
    vaultc::provenance::validate_stored_graph(&attribution_erasure)
    ...
    vaultc::provenance::validate_stored_graph(&swapped)
...
fn virtual_envelope_pagination_cursor_and_directory_pack_queries_are_identical() {
    ...
    vaultc::pack::create_pack(&artifact, &pack, compiler.policy().output.zstd_level)
        .expect("create provenance query VaultPack");
    ...
    for (path, role) in [
        ...
        assert_eq!(
            vaultc::canonical::to_canonical_json(&directory)
                .expect("canonical directory explanation"),
            vaultc::canonical::to_canonical_json(&packed).expect("canonical packed explanation")
        );
    ...
    let stored_plan = compiler
        .explain_provenance(&artifact, ".vaultc/plan.json")
    ...
    for query in [
        ...
        ProvenanceQuery::artifact_path(".vaultc/plan.json").with_cursor(valid_cursor.clone()),
    ] {
        assert!(matches!(
    ...
    assert!(matches!(
        ...
        Err(VaultcError::ResourceLimit(_))
    ));
...
fn page_max_bytes_bounds_the_complete_serialized_response() {
    ...
    assert_eq!(
        exact_bytes,
        vaultc::canonical::to_canonical_json(&one_record)
    ...
    for (source_id, fixture, expected_subject) in [
        ...
        let decisions = conflicts
            ...
            .map(|conflict| ConflictDecision {
                ...
                resolver: "qa-typed-provenance".into(),
                policy_version: "test-v1".into(),
