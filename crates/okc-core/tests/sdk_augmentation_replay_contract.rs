...
use okc_core::canonical::{canonical_hash, to_canonical_json};
use okc_core::identity::{ContentHash, DocumentId};
use okc_core::pack::create_pack;
use okc_core::provider::CancellationToken;
use okc_core::{
    ApprovalDecision, ApprovalLog, CompilerPolicy, DraftPlan, KnowledgeAugmentor, OkcCompiler,
    OkcError, SourceSpec,
};
use okc_protocol::{
...
) -> okc_core::Result<RecordedAugmentation> {
...
fn assert_replay_rejected(compiler: &OkcCompiler, plan: &DraftPlan, bytes: &[u8], case: &str) {
...
fn all_selection_on_zero_document_plan_fails_before_provider_call() {
    ...
    assert!(matches!(error, OkcError::InvalidConfig(_)));
...
fn offline_replay_is_provider_free_and_byte_identical() {
    ...
    let live_pack = temporary.path().join("live.okcpack");
    let replay_pack = temporary.path().join("replay.okcpack");
...
fn approval_rejects_nonempty_validations_without_transcript() {
    ...
    assert!(matches!(error, OkcError::ApprovalStale(_)));
    ...
    context
        ...
        .approve(
            context.plan.clone(),
            okc_core::ValidatedProposals::default(),
            ApprovalLog::default(),
        )
...
fn sdk_augmentation_cancellation_is_atomic() {
    ...
    assert!(matches!(error, OkcError::Provider(_)));
...
fn recorded_augmentation_rejects_noncanonical_jsonl() {
    ...
    for (name, bytes) in [
        ...
        assert!(matches!(error, OkcError::Provider(_)), "{name}: {error}");
    }
}
