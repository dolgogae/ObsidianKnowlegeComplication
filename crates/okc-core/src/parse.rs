...
use crate::ir::{
    Asset, BaseArtifact, Block, BlockKind, CanonicalWorkspace, Canvas, CanvasEdge,
    CanvasFileReference, CanvasNode, CanvasReferenceResolution, Document, FileKind,
    IR_SCHEMA_VERSION, Link, LinkResolution, LinkSyntax, Section, SourceFile,
};
...
pub(crate) fn parse_file(
    ...
    match source_file.kind {
        ...
        FileKind::Asset => {
            let asset_id =
                AssetId::from_parts("okc:asset:v2\0", &[source_file.content_hash.as_bytes()]);
...
pub(crate) fn parse_markdown(
    ...
    let body_hash =
        ContentHash::from_domain_bytes("okc:body:v2\0", structural_projection.as_bytes());
...
fn parse_frontmatter(
    ...
    let Some(raw) = raw else {
        return Ok((
            ...
            Vec::new(),
            None,
            ContentHash::from_domain_bytes("okc:frontmatter:v2\0", b"null"),
        ));
    };
    match serde_yaml_ng::from_str::<serde_yaml_ng::Value>(raw) {
        Ok(yaml) => {
            ...
            Ok((
                ...
                tags,
                title,
                ContentHash::from_domain_bytes("okc:frontmatter:v2\0", &canonical),
            ))
        }
        Err(error) if error.to_string().contains("duplicate entry") => {
            Err(OkcError::MalformedInput {
                path: path.to_owned(),
...
fn opaque_frontmatter(
    ...
    (
        ...
        Vec::new(),
        None,
        ContentHash::from_domain_bytes("okc:frontmatter-opaque:v2\0", raw.as_bytes()),
    )
}
...
#[allow(
    ...
    reason = "one raw-byte line scan keeps section, block, fence, and source-span offsets coherent"
)]
fn scan_blocks(
    ...
    for line in raw_lines(body) {
        ...
        if let Some((marker, minimum, fence_start)) = fence {
                    ...
                    .is_some_and(u8::is_ascii_whitespace)
            {
                ...
                heading_stack.truncate(hash_count.saturating_sub(1));
                heading_stack.push(heading.clone());
                let section_index = sections.len() as u64;
                let section_id = SectionId::from_parts(
                    "okc:section:v2\0",
                    &[
                        document_id.hash().as_bytes(),
                        &section_index.to_be_bytes(),
                        heading.as_bytes(),
                    ],
                );
    ...
    assign_section_blocks(&mut sections, &blocks);
    (sections, blocks)
}

fn assign_section_blocks(sections: &mut [Section], blocks: &[Block]) {
    for index in 0..sections.len() {
        let start = sections[index].span.byte_start;
        let level = sections[index].heading_level;
        let end = sections
            .iter()
            .skip(index + 1)
            .find(|section| section.heading_level <= level)
            .map_or(u64::MAX, |section| section.span.byte_start);
        sections[index].blocks = blocks
            .iter()
            .filter(|block| block.span.byte_start >= start && block.span.byte_start < end)
...
fn push_block(
    ...
    let kind = classify_block_kind(kind, slice);
    let content_hash = ContentHash::from_domain_bytes("okc:block-content:v2\0", slice.as_bytes());
...
fn classify_block_kind(default: BlockKind, slice: &str) -> BlockKind {
    if default != BlockKind::Paragraph {
        return default;
    }
...
fn is_numbered_list_item(value: &str) -> bool {
    value
        .split_once('.')
...
fn parse_canvas(
    ...
    let (nodes, edges) = typed_canvas_members(&value, &source_file.logical_path)?;
    Ok(Canvas {
        canvas_id,
        source_file,
        value,
...
#[allow(
    clippy::too_many_lines,
    reason = "typed Canvas node and edge validation preserves one strict unknown-field boundary"
)]
fn typed_canvas_members(
    value: &Value,
    logical_path: &str,
) -> Result<(Vec<CanvasNode>, Vec<CanvasEdge>)> {
    let root = value.as_object().ok_or_else(|| OkcError::MalformedInput {
    ...
    for value in root
        ...
        .flatten()
    {
        let object = value.as_object().ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.into(),
                ...
                .map(|value| {
                    value
                        ...
                        .ok_or_else(|| OkcError::MalformedInput {
                            path: logical_path.into(),
                            reason: format!("Canvas node `{field}` must be a string"),
                        })
                })
                .transpose()
        };
        let integer = |field: &str| -> Result<Option<i64>> {
            object
                .get(field)
                .map(|value| {
                    value.as_i64().ok_or_else(|| OkcError::MalformedInput {
                        path: logical_path.into(),
                        reason: format!("Canvas node `{field}` must be an integer"),
                    })
                })
                .transpose()
        };
        let id = string("id")?.ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.into(),
            reason: "Canvas node is missing string `id`".into(),
        })?;
        let node_type = string("type")?.ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.into(),
            reason: format!("Canvas node `{id}` is missing string `type`"),
        })?;
        ...
        .flatten()
    {
        let object = value.as_object().ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.into(),
            reason: "JSON Canvas edges must be objects".into(),
        })?;
        let required = |field: &str| -> Result<String> {
            object
                ...
                .ok_or_else(|| OkcError::MalformedInput {
                    path: logical_path.into(),
                    reason: format!("Canvas edge is missing string `{field}`"),
                })
        };
        let optional = |field: &str| -> Result<Option<String>> {
            object
                .get(field)
                .map(|value| {
                    value
                        ...
                        .ok_or_else(|| OkcError::MalformedInput {
                            path: logical_path.into(),
                            reason: format!("Canvas edge `{field}` must be a string"),
                        })
                })
                .transpose()
        };
        ...
        edges.push(CanvasEdge {
            id: required("id")?,
            from_node: required("fromNode")?,
            to_node: required("toNode")?,
            from_side: optional("fromSide")?,
            to_side: optional("toSide")?,
...
pub(crate) fn parse_canvas_json(
    ...
    let value =
        crate::parse_json_strict(text.as_bytes()).map_err(|error| OkcError::MalformedInput {
            path: logical_path.to_owned(),
...
pub(crate) fn canvas_file_references(
    ...
    let nodes: &[Value] = match object.get("nodes") {
        Some(nodes) => nodes.as_array().ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.to_owned(),
    ...
    for node in nodes {
        let node = node.as_object().ok_or_else(|| OkcError::MalformedInput {
            path: logical_path.to_owned(),
            reason: "JSON Canvas nodes must be objects".into(),
        })?;
        let node_id =
...
mod tests {
    ...
    fn fixture_file(path: &str, bytes: &[u8]) -> SourceFile {
        ...
        SourceFile {
            ...
            raw_sha256: hex::encode(sha2::Sha256::digest(bytes)),
            content_hash: hash,
        }
    }
